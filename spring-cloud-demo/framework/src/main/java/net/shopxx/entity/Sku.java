 ...
 * FileId: hPr+YY7e1cC+p7uWfs0qa9emP+2rcftWtpu0hiin+ag=
 */
...
public class Sku extends BaseEntity<Long> {
	...
	public BigDecimal getPlatformCommission(net.shopxx.entity.Store.Type type) {
		...
		//先判断商品是否设置了平台分佣
		Double productRate=getProduct().getGeneralRate();
		if(productRate!=null && productRate>0) {
			commission = getPrice().multiply(new BigDecimal(String.valueOf(productRate)).movePointLeft(2));
		}else if (type != null && getProduct() != null && getProduct().getProductCategory() != null) {
			ProductCategory productCategory = getProduct().getProductCategory();//获取商品分类
			...
			} else if (net.shopxx.entity.Store.Type.SELF.equals(type) && productCategory.getSelfRate() > 0) {//自营商品
