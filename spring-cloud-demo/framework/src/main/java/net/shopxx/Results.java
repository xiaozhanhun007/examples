 ...
 * FileId: Zov1XGmJmqE536WNox9QaFcaPbN9OatkxlI0Ukq4daY=
 */
package net.shopxx;
...
import net.shopxx.framework.core.model.ResultDto.Type;
...
public final class Results {
	...
	public static void status(HttpServletResponse response, HttpStatus httpStatus,Results.Type type,Object data, String message, Object... args) {
		Assert.notNull(response, "[Assertion failed] - response is required; it must not be null");
		Assert.notNull(httpStatus, "[Assertion failed] - httpStatus is required; it must not be null");
		Assert.hasText(message, "[Assertion failed] - message must have text; it must not be null, empty, or blank");

		Map<String, Object> result = new HashMap<String, Object>();
		result.put(MESSAGE_KEY, SpringUtils.getMessage(message, args));
		result.put(DATA_KEY, data);
		result.put("type", type);
		status(response, httpStatus, result);
	}
	...
	public static void ok(HttpServletResponse response,Results.Type type,Object data, String message, Object... args) {
		status(response, HttpStatus.OK,type,data,message, args);
	}
	
	public static void result(HttpServletResponse response,Results.Type type,Object data, String message, Object... args) {
		status(response, HttpStatus.OK,type,data,message, args);
	}
